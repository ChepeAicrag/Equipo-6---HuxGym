...
class Customer(models.Model):
    ...
    image = models.ImageField(
        upload_to=upload_load, max_length=255, blank=True, null=True, default='default.jpg',)
...
class HistoryClinic(models.Model):
    ...
    bloodType = models.CharField(
        max_length=4, blank=False, null=False, choices=bloodTypes, default='O+')
    status_delete = models.BooleanField(default=False)

    customer_id = models.ForeignKey(Customer, on_delete=models.CASCADE)
    nutritionalSituation_id = models.ForeignKey(
        NutritionalSituation, null=True, on_delete=models.SET_NULL, blank=True)

    typeExtraInformation = models.ManyToManyField(
        TypeExtraInformation, through='TypeExtraInformation_HistoryClinic')
...
class TypeExtraInformation_HistoryClinic(models.Model):
    ...
    typeExtraInformation_id = models.ForeignKey(
        TypeExtraInformation, on_delete=CASCADE)
...
class BodyAttribute_HistoryClinic(models.Model):
    bodyAttribute_id = models.ForeignKey(
        BodyAttribute, on_delete=models.CASCADE)
    historyClinic_id = models.ForeignKey(
        HistoryClinic, on_delete=models.CASCADE)
